...
\begin{figure}[htbp]
  \centering
  \fbox{\begin{minipage}{0.88\linewidth}\centering\vspace{4cm}
    \textit{[Conceptual diagram: Sovereignty--Readiness frontier. X-axis: Readiness score (0--100). Y-axis: SovScore (0--100). Points: AWS (bottom-right), Locally Hosted (top-left), OVHcloud (upper-centre-right), T-Systems OTC (centre-right, lower than OVHcloud on Y), Hetzner (upper-centre-left), Elastx/Infomaniak (upper-left), STACKIT (centre-left). Dashed Pareto frontier curve connecting locally hosted to OVHcloud.]}
  \vspace{4cm}\end{minipage}}
  \caption{Conceptual sovereignty--readiness frontier. Each provider occupies a position reflecting its trade-off between managed-service breadth (readiness) and freedom from non-EU dependencies (sovereignty). No provider currently sits at the ideal upper-right corner.}
